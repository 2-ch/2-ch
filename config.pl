...
use constant S_ANONAME => '名無しさん＠お腹いっぱい。';			# Defines what to print if there is no text entered in the name field
use constant DEFAULT_STYLE => 'Pseud0ch';		# Default CSS style title
...
#use constant AUTOCLOSE_POSTS => 1000;			# Maximum number of posts before a thread closes. 0 to disable.
use constant AUTOCLOSE_DAYS => 10;				# Maximum number of days with no activity before a thread closes. 0 to disable.
#use constant AUTOCLOSE_SIZE => 0;				# Maximum size of the thread HTML file in kilobytes before a thread closes. 0 to disable.
#use constant MAX_RES => 1000000000;			# Maximum topic bumps
...
use constant MAX_LINES_SHOWN => 9999999999999999999999999999999;			# Max lines of a comment shown on the main page (0 = no limit)
...
use constant COOKIE_PATH => 'current';			# Path argument for cookies ('root': cookies apply to all boards on the site, 'current': cookies apply only to this board, 'parent': cookies apply to all boards in the parent directory) - does NOT apply to the style cookie!
#use constant STYLE_COOKIE => 'karehastyle';	# Cookie name for the style selector.
use constant ENABLE_DELETION => 0;				# Enable user deletion of posts. (0: no, 1: yes)
...
use constant DEFAULT_MARKUP => 'none';			# Default markup format, of the above.
